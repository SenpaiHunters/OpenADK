...
public class TabsManager: TabManagerProtocol {
    ...
    public func closeActiveTab() {
        if let currentTab = self.state?.currentSpace?.currentTab {
            currentTab.closeTab()
        }
    }
    ...
    public func getLocation(_ location: String) -> TabLocationProtocol? {
        ...
        allLocations = allLocations + self.globalLocations
    ...
    public func createNewTab(url: String, frame: CGRect = .zero, configuration: WKWebViewConfiguration = AltoWebViewConfigurationBase(), location: String = "") {
        guard let state = self.state else {
            return
        }
        
        guard let tabLocation = self.getLocation(location) else {
            return
        }
